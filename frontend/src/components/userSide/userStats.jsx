...
const UserSideStats = () => {
    ...
    const handleEdit = (index) => {
        ...
        const userValue = performance.find((item) => item.month === month)?.ValueArray?.[index]?.UserValue;
      ...
      const handleSave = (index, item) => {
        ...
        const updatedPerformance = performance.map((performanceItem) => {
          if (performanceItem.month === updatedMonth) {
            const updatedValueArray = performanceItem.ValueArray.map((valueItem, valueIndex) => {
              if (valueIndex === index) {
                return {
                  ...valueItem,
        ...
        // Update API
        axios
          .put(`http://localhost:3012/employee/${index}`, {
      ...
      const handleDelete = (index) => {
        const updatedPerformance = performance.map((performanceItem) => {
          const updatedValueArray = performanceItem.ValueArray.filter(
            (valueItem, valueIndex) => valueIndex !== index
          );
          return {
            ...performanceItem,
            ValueArray: updatedValueArray,
        ...
        // Update local storage
        const updatedStatData = data.filter((dataItem, dataIndex) => dataIndex !== index);
        localStorage.setItem("statData", JSON.stringify(updatedStatData));
        ...
        <Container>
            <Box>
                ...
                <FormControl sx={{ m: 1, minWidth: "100%" }}>
                    <InputLabel id="demo-simple-select-autowidth-label">Monthly Performance</InputLabel>
                    <Select
                ...
                >
                    {data.map((item, index) => {
                        const userValue =
                            performance.find((item) => item.month === month)?.ValueArray?.[index]?.UserValue;

                        return (
                            <Grid key={item.statValue}>
                                <Card sx={{ minWidth: "300px", boxShadow: "20px 20px 40px grey", m: 3 }}>
                                    <CardMedia component="img" height="60" style={{ backgroundColor: "purple" }} />
                                    <CardContent>
                                    ...
                                    <Box sx={{ width: '100%', marginLeft: 6 }}>
                                            {!isEditMode || editIndex !== index ? (
                                                <>
                                                    <Typography>
                                                        Rate Value: {userValue} out of {Number(item.statRange)}
                                                    </Typography>
                                                    <Slider
                                                        min={0}
                                                        max={Number(item.statRange)}
                                                        value={
                                                            performance.find((item) => item.month === month)?.ValueArray?.[index]
                                                                ?.UserValue || 0
                                                        }
                                                ...
                                                <Slider
                                                    ...
                                                    style={{ marginLeft: 6, marginTop: 10, width: '80%' }}
                                                    valueLabelDisplay="auto"
                                                    onChange={(event, newValue) => handleSliderChange(item, newValue)}
                                                />
                                            )}
                                        </Box>
